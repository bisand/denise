...
- **There is no grid widget.** There are eight widgets — label, button, panel,
  text field, checkbox, toggle, radio group, progress bar. A row is a full-width `Button` with the cell `Label`s placed on top of it;
...
- **Eight widgets.** Label, button, panel, text field, checkbox, toggle, radio
  group, progress bar. Everything
