...
denise = "0.30"
denise-ui = "0.30"
denise-winit = "0.30"    # develop on a desktop
# denise-drm = "0.30"    # ship on a display with no compositor
# denise-image = "0.30"  # decode PNG, JPEG, GIF and BMP
```
...
- **There was no grid widget when this example was written** — and it remains a
  lesson in how the widgets compose. There are twenty-eight now, `Table` among
  ...
  select, image, rating, avatar, table, timeline, carousel, collapse, video, tree,
  text area. A row is a full-width `Button` with the cell `Label`s placed on top of it;
...
draw: the built-in 8×8 bitmap font (0 KB, always there), TrueType via `ab_glyph`
(+65 KB, `truetype`), and full shaping via `cosmic-text` (+3.1 MB, `shaping`).
...
- **No undo** in `TextInput`. Everything else a field is expected to do is
  there: focus takes the whole value, a press places the caret, a second takes
  the word, a third takes everything, Ctrl or Option and an arrow move by word,
  and ⌘C, ⌘X and ⌘V go through the application the way `TextArea`'s do, because
  a panel has no clipboard for a widget to reach. What is missing is the undo
  stack — `TextArea` has one, and a field does not.
...
- CI runs `cargo deny` over the whole tree — advisories, licences, sources and
  ...
  publish; a weekly run catches the one that lands in a month nobody pushed.
  Every build is `--locked`, every action is pinned to a commit, and Dependabot
  keeps both moving.
