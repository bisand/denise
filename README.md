...
- **There is no grid widget.** There are four widgets — label, button, panel, text
  field. A row is a full-width `Button` with the cell `Label`s placed on top of it;
...
- **Four widgets.** Label, button, panel, text field. Everything else is assembled
  from them, as `table-editor` shows.
