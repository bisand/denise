...
- **There is no grid widget.** There are seven widgets — label, button, panel,
  text field, checkbox, toggle, radio group. A row is a full-width `Button` with the cell `Label`s placed on top of it;
...
- **Seven widgets.** Label, button, panel, text field, checkbox, toggle, radio
  group. Everything
