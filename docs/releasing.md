...
All nineteen crates share one version and go to crates.io together.
...
It did exactly that to v0.25.0. It had gone unnoticed once before, at v0.24.1
— the first release after the trigger was added, which published only because
the gate happened to look before the check appeared. Dispatching from `main`
after the publish is allowed, is late enough that the gate has finished, and is
late enough for the downloads to exist behind the links the page builds.
...
These nineteen crates are one product split up for compilation reasons — nobody uses `denise-render` without `denise` — so that is the property worth keeping. The cost is that a crate gets a new version when nothing in it changed, which matters to somebody depending on exactly one of them. Nobody does yet.
...
| **A full dry run first** | Packages and verifies all nineteen before anything real goes |
...
| Version | |
|---|---|
| [v0.30.0](https://github.com/bisand/denise/releases/tag/v0.30.0) | An idle window costs nothing, and a face costs what its file does. `denise-text`'s `truetype` tier moves from `fontdue` to `ab_glyph`, which reads a glyph's outline when it is first drawn rather than every outline when the face is opened: FiraCode Nerd Font's twelve thousand glyphs came to 60 MB and most of a second on an Atom before anything was drawn, DejaVu Sans to 20 MB, and both now cost about their file and open in under a millisecond. The drawing is the same — over printable ASCII and a few non-Latin glyphs at seven sizes, one glyph in 728 differs, by one pixel, and total ink by half a percent, and squint's window is pixel for pixel what it was but for antialiasing nobody can see — and the tier is 65 KB of static binary rather than 145; one uncached glyph rasterises about 1.4× slower, which the atlas pays once per glyph and size. A caret in `TextInput` and `TextArea` blinked for as long as it had focus, two wakes and two frames a second in a window left open, and now blinks for ten seconds after the last keystroke or move, GTK's default, then rests lit asking for nothing. `Waker`, handed to every application through `DeniseApp::set_waker`, lets a socket or another thread ask the loop for a frame, which is what an application otherwise polls `next_frame_in` for. `DeniseApp::presenting` says whether the window got `Present::Gpu` or `Present::Software`, so a machine where no GPU could present can be remembered rather than loading LLVM and Mesa to find out on every run. Both trait methods have defaults. One break: `TrueTypeSource::font()`, which handed out the `fontdue::Font`, is gone, and `from_vec` and `from_static` are new. Found in squint, which held 138 MB with no document open and woke eight times a second doing nothing; it now holds 18 MB, starts on 0.13 s of CPU rather than 1.9, and wakes eight times per thirty seconds. Clean run |
| [v0.28.0](https://github.com/bisand/denise/releases/tag/v0.28.0) | A window says which application it belongs to. `WindowConfig::app_id` names the application to the desktop — Wayland's app_id and X11's `WM_CLASS`, through the one name winit keeps for both — which is what ties a window to its desktop entry, so a launcher and a task bar show its name and icon, and what a window manager's rules match; without it a Wayland compositor has nothing to go on. Ignored on macOS and Windows, which know an application by its bundle and its executable. Found in squint, whose windows Hyprland listed with an empty class. Additive except the new `WindowConfig` field, which a struct literal without `..Default::default()` must name. Clean run |
| [v0.27.0](https://github.com/bisand/denise/releases/tag/v0.27.0) | A window that asks for the GPU and draws in software where there is none. `Present::GpuOrSoftware` opens a swapchain where a GPU can present to the window and the software surface where none can — a virtual machine, a remote display, a missing driver — decided for each window as it opens, with the reason on stderr; the window is kept and only what draws into it changes. It could not be done by the application: catching `Error::Gpu` from `run_with` and running again in software fails with `EventLoop(RecreationAttempt)`, because winit makes one event loop per process. Found in squint, which never opened on the Alpymist VM. Additive, except to a `match` on `Present` with no wildcard, which must name the new variant. Clean run |
| [v0.26.0](https://github.com/bisand/denise/releases/tag/v0.26.0) | A text area that scrolls by the pixel. `TextArea::with_smooth_scroll`, `set_smooth_scroll` and a `smooth-scroll` form property move the text as far as the wheel says rather than a line once a line's worth has built up — a slow trackpad in squint stepped a line at a time. The view is a top line and the pixels of it above the edge: paint starts that line above the view and draws a row more, a click adds the offset back before it chooses a line, and the gutter's numbers are clipped so a half-gone one cannot draw over what is above the editor. A fast wheel goes exactly as far as it does by lines, both ends stop on a whole line, and whatever moves the view by lines — a jump, a page, the thumb, a found range, the caret — lands on one. Off unless asked for, so every text area already built scrolls as it did. All additive. Clean run |
| [v0.25.0](https://github.com/bisand/denise/releases/tag/v0.25.0) | A window opens where it was closed. `WindowConfig` gains `position` and `maximized`, and `InputEvent` gains `SurfaceMoved` — the other half of `SurfaceResized`: that one says how big the window is, this one says where it is and what the window manager has done with it. Keep what the last run was told, hand it to the next, and the window comes back. The units are the ones each fact is really kept in: the size logical, so it covers the same desk on a display of another DPI; the corner physical, because a desk spanning displays of different DPI has no single logical grid to name a point in. Two things that would have made it quietly wrong. On macOS the position *attribute* places the content below the title bar while `outer_position` — what `SurfaceMoved` reports — is the frame's own corner, so a window remembered through that pair climbs its own title bar's height every time it opens; the corner is set again with `set_outer_position` once the window exists, in the units it will be read back in. And a corner on a display that has since been unplugged is ignored rather than honoured, a machine reporting no displays at all being taken at its word. Found in squint, which had no way to open the way it closed. All additive except the two new `WindowConfig` fields, which a struct literal without `..Default::default()` must name. Cut twice — the first attempt stopped on a rustdoc warning about a redundant link target, before anything was uploaded — and then held up a third time by the gate, which counts the site's `deploy` among CI: the Website workflow runs on `release: published`, so it runs at the tag, and the `github-pages` environment only lets `main` deploy. Dispatching the site from `main` turned that check green and the publish was re-run in place |
| [v0.24.1](https://github.com/bisand/denise/releases/tag/v0.24.1) | A line far longer than the view stops taking the GPU down with it. A text area handed one — a binary file opened as text, a minified document with no line breaks in it — built geometry for every glyph of the line and not only the ones on screen, and on a painter that makes a quad per glyph at six vertices apiece a 587 KB line came to a 321 MB vertex buffer, past what wgpu will take in one, so the frame died on a validation error. `TextEngine::draw_line` now rasterises and hands over only the glyphs the canvas would keep anything of, horizontally and vertically, with a four-em margin for what a glyph reaches outside its own advance. The pixels are the same, and so is the width it returns — of the whole line and not the part drawn, which is what a text area measures to know how far it scrolls sideways. Laying the line out is still the whole of it and is the cheap half: an advance apiece out of the glyph cache, against rasterising and uploading. Found in squint, on a 393 MB disk image. No API change. Clean run, main fast-forwarded to the bump — and the first release to leave a red `deploy` behind it, the site's `release: published` trigger having been added minutes after 0.24.0 went out. Nobody noticed until it stopped v0.25.0 |
| [v0.24.0](https://github.com/bisand/denise/releases/tag/v0.24.0) | Text editing: everything a person expects of a field, and the one thing the editor's view got wrong. `TextInput` had a caret and no pointer handling at all, so a press focused it and left the caret where it had been. Now focus takes the whole value, so tabbing into a setpoint and typing replaces it, while a press still lands the caret where the finger did, being delivered after the focus it caused; a press places, a second takes the word, a third takes the field and a fourth starts again; dragging extends from the press, by mouse or finger; Ctrl or Option and an arrow move by word and Command and an arrow goes to an end, Shift extending with all of them; and copy, cut and paste go through the same `ClipboardRequest` the editor uses, so a host answering one answers both — a password field refusing copy and cut, because the mask exists so the value cannot be read, and a paste taking one line, because a field is one line. Undo is what a field still has not got. In the editor: a third press takes the line's text and not the newline after it, so a Backspace there leaves an empty line rather than pulling the next one up; the same two spellings of word motion; and the view stops scrolling sideways, which it had done without limit — a wheel flick to the right left the text behind and took a long scroll back — going now as far as the widest line it has drawn plus the caret and a character of daylight, because a document read a line at a time cannot be asked how wide the file is. A bar along the bottom is a share of the track as wide as the view is of the text, pages on a click and follows its thumb, and is absent rather than greyed when the text fits, so an editor of short lines looks exactly as it did. The word rule is one rule: `is_word` is shared, so a double click takes the same run in a field as in an editor. Off the API, the site at [bisand.github.io/denise](https://bisand.github.io/denise/) — the gallery and the record editor compiled to `wasm32-wasip1` and running in the page with every repainted rectangle outlined, twenty-eight widgets, the programs built with it, the designer's downloads and six guides — and the documentation that disagreed with the code corrected rather than left: the widget count, the crate count (`denise-arrange` was in no table), three keyboard layouts, `PresentMode::Vsync` as the DRM default, the ActiveX type library that exists, `denise-win32` having run, and `docs/forms.md` gaining `menubar` and `text-area`, which loaded without a section on the page meant to be the schema. All additive. Clean run, main fast-forwarded to the bump |
| [v0.23.0](https://github.com/bisand/denise/releases/tag/v0.23.0) | A tab strip people work in. `Tabs::with_events` reports what is done to a tab as a `TabEvent` — `Selected`, `Close` from its close button or a middle click, `Moved` once when a dragged tab is let go, `Activated` on a double click for renaming, and `Menu` with the point of a right click — and still decides nothing: a close is asked for and never done, and `tab_rect` says where a rename field goes. Dragging is the one thing it does itself, trading places at tab centres rather than edges so neighbours of different widths do not flicker back and forth. A tab can take a colour, which tints it and marks its top edge, with its labels derived to stay readable on whatever tint that makes; close buttons keep their room on every tab so the row does not shift under the pointer; a row wider than its strip slides to keep the selected tab in view. A strip made with `Tabs::new` is unchanged, so forms, the designer and the gallery are too. All additive. Found in squint, which needed a row of documents rather than a row of sections. Clean run, main fast-forwarded to the bump |
| [v0.22.0](https://github.com/bisand/denise/releases/tag/v0.22.0) | A text area, and an application that hears ⌘Q. `TextArea`, the twenty-eighth widget, holds no string. It edits through a `TextDocument` trait that is asked only for the lines about to be painted, so a file too big to load can sit behind it; `TextBuffer` is what a form gets. It has a scrollbar, `go_to` to jump to a line, `select_range`, and a `highlights` hook for marking find matches. `DeniseApp::exiting` is the last call an application gets, once per window however it ends, including macOS Quit, Dock Quit and logout, which end the process with no close request and no return from `run`. The docs stop sending saving to a `CloseRequested` that a closing window may never read. All additive. Clean run, except that main had moved on before the bump could fast-forward it, so the bump lived on the tag alone until it was merged by hand |
| [v0.21.1](https://github.com/bisand/denise/releases/tag/v0.21.1) | A menu leaves no row lit once the pointer has left it. After a submenu had opened, a row crossed on the way out stayed lit and the submenu stayed open. Found in ctail's File ▸ Recent. No API change. Clean run |
| [v0.21.0](https://github.com/bisand/denise/releases/tag/v0.21.0) | Menus behave like a desktop's. One popup scene holds the whole cascade: a submenu opens when the pointer rests on its row and closes once the pointer settles on another, sliding along the bar switches menus, and a press outside dismisses. `MenuItem::separator` and `MenuItem::submenu` join the rows. One break: `MenuEvent` replaces the bare index. A widget that scrolls itself also has its rows moved rather than redrawn. Clean run, after a first attempt fell over a dead rustdoc link in `denise-wgpu` |
| [v0.20.0](https://github.com/bisand/denise/releases/tag/v0.20.0) | A form is a file, and there is a designer for it. Three crates join: `denise-forms` reads a `.dform` into a `Ui` at run time and also checks, renders, formats and generates Rust from one, with a fuzzed corpus behind the parser and the builder; `denise-arrange` lays out by intent once a widget can be asked how big it wants to be; `denise-wgpu` is a second painter, on the GPU, which exists because painting became a trait and the rasteriser stopped being the definition of drawing — a glyph crosses it as a rectangle of its page and a picture with an identity, so a GPU uploads each once rather than per frame, and the numbers against the rasteriser at two resolutions are measured rather than claimed. The designer is a program and so a download rather than a `cargo install`: three panes, a canvas that selects, moves, resizes, bands, reparents and reorders, an inspector that edits, an outline whose tree is tab order because tab order is file order, align/distribute/same-size/group, cut/copy/paste/duplicate, F5 to run the form, a text editor on the same file, and undo and redo exact to the byte because the file is the document. `denise-ui` gains a menu bar and its popup, a `Tree`, a select and a section that need no message, and widgets that describe what they are and what properties they have — which is what lets an inspector exist at all. Two breaks: anchors and docking in the pass the tree already ran (#116), and the property inspector editing (#125). Smaller: the horizontal wheel went the wrong way under winit on every application (#166), `denise-text` gained a default face (#152), a hidden node really does take its descendants (#169), and `WindowConfig::fill_monitor` opens a window at the monitor's size for a panel a window manager is about to fullscreen anyway. Cut twice — the first attempt stopped on red CI before uploading anything, which is the gate doing its job |
