...
All fifteen crates share one version and go to crates.io together.
...
These fifteen crates are one product split up for compilation reasons — nobody uses `denise-render` without `denise` — so that is the property worth keeping. The cost is that a crate gets a new version when nothing in it changed, which matters to somebody depending on exactly one of them. Nobody does yet.
...
| **A full dry run first** | Packages and verifies all fifteen before anything real goes |
