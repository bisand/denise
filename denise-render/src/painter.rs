...
//!
//! [`Canvas`] is one implementation of it — the software rasteriser this crate
...
/// is twelve o'clock, clockwise positive, [`TURN`](crate::TURN) is a full circle.
///
...
pub trait Painter {
    ...
    ///
    /// See [`Canvas::fill_star`] for what the binary-turn rounding costs.
    fn fill_star(
        &mut self,
        centre: Point,
        outer_radius: i32,
        ...
        use crate::polygon::MAX_VERTICES;
        use crate::rounded::{COORD_LIMIT, to_fx};
        ...
        for (i, vertex) in vertices.iter_mut().enumerate().take(count) {
            ...
            let step = (i as i64 * crate::TURN as i64 + count as i64 / 2) / count as i64;
...
/// trait's for resolution -- so a widget body written against [`Canvas`] reads
...
impl<'a> Pen<'a> {
    ...
    /// was given, exactly as with [`Canvas::with_clip`].
