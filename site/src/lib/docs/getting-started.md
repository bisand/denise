...
`hello` draws in the built-in 8×8 bitmap font because it never asks for another. Real faces are the `truetype` feature (about 270 KB) and full shaping is `shaping` (about 3.1 MB), both passed through `denise-ui`. [How it works](../how-it-works/) has the trade-off.
