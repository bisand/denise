...
`hello` draws in the built-in 8×8 bitmap font because it never asks for another. Real faces are the `truetype` feature (about 270 KB) and full shaping is `shaping` (about 3.1 MB), both passed through `denise-ui`; a product that ships one face can instead bake it at build time (`bake`, in a `build.rs`) and link no font parser at all. [How it works](../how-it-works/) has the trade-off.
